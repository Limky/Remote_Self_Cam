...
public class FragmentImageDetail extends FragmentBase {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        imageDetailView.setFocusableInTouchMode(true);
        imageDetailView.requestFocus();
        imageDetailView.setOnKeyListener(new View.OnKeyListener() {
            @Override
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                com.sqisoft.remote.util.Log.i("tag", "keyCode: " + keyCode);
                if( keyCode == KeyEvent.KEYCODE_BACK ) {
                    Toast.makeText(getActivity(),"백버튼 인터셉터",Toast.LENGTH_LONG).show();
                    com.sqisoft.remote.util.Log.i("tag", "onKey Back listener is working!!!");

                    manager = getFragmentManager();
                    transaction = manager.beginTransaction();
                    transaction.replace(R.id.replacedLayout, new FragmentGallery()).commit();
                    setTitle("셀카 갤러리");
